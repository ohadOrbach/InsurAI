...
4. Task parsing component (if enabled): Load `.cursor/rules/parse-and-run-tasks.mdc`
...
flowchart TD
    ...
    LoadStart --> CheckTasks{Task parsing enabled?}
    LoadRIPER --> CheckTasks
    
    CheckTasks -->|Yes| LoadTasks[Load parse-and-run-tasks.mdc]
    CheckTasks -->|No| LoadMemory[Load Memory Bank]
...
└── tasks/                 # Task parsing and execution tracking (when enabled)
    ├── tasks.json         # Task definitions parsed from PRD
